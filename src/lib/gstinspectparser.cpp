...
GstStudio::GstInspectParser::GstInspectParser(QObject* parent)
	: QObject(parent)
	, m_process(new QProcess(this))
{
	connect(m_process, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
		this, &GstInspectParser::onProcessFinished);
}
...
GstElement GstStudio::GstInspectParser::parseElementDetails(const QString& output)
{
	...
	// Parse factory details - looking for lines like "dv1394src:   Long-name                Firewire (1394) DV video source"
...
void GstStudio::GstInspectParser::parseProperties(const QString& section, GstElement& element)
{
	...
	for (const QString& line : std::as_const(lines)) {
		...
		if (propMatch.hasMatch()) {
			...
			static QRegularExpression typeRegex(R"(^(\w+(?:\s+\w+)*)\.\s*(?:Range:\s*([^D]+?))?\s*(?:Default:\s*(.+))?$)");
...
void GstStudio::GstInspectParser::parsePadTemplates(const QString& section, GstElement& element)
{
	...
	for (const QString& line : std::as_const(lines)) {
		...
		if (inCapabilities && (trimmedLine.startsWith("video/") || trimmedLine.startsWith("audio/") || trimmedLine.startsWith("application/") || trimmedLine.startsWith("text/") || trimmedLine.startsWith("image/") || trimmedLine.contains("format:") || trimmedLine.contains("systemstream:"))) {
		...
		if (inCapabilities && (trimmedLine.contains("Element has") || trimmedLine.contains("URI handling") || trimmedLine.contains("Pads:"))) {
			inCapabilities = false;
		}
	}
...
QString GstStudio::GstInspectParser::extractSection(const QString& text, const QString& sectionName)
{
	...
	QStringList nextSections = { "Factory Details:", "Pad Templates:", "Element Properties:", "Element Signals:", "Element Actions:" };
